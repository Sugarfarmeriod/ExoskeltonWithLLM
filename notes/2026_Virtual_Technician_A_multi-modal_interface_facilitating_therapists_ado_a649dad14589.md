...
- 题目：Virtual Technician: A multi-modal interface facilitating therapists' adoption of rehab robots
- 作者：未提供
- 年份：2026
- 期刊/会议：未提供
...
语义/治疗师输入可作为高层参数或模式选择接口，但优先级低于优化器和状态估计器；涉及本研究不使用的 EMG/IMU，仅作方法参考，不作为直接实现模板
...
- 当前自动判断：语义/治疗师输入可作为高层参数或模式选择接口，但优先级低于优化器和状态估计器；涉及本研究不使用的 EMG/IMU，仅作方法参考，不作为直接实现模板
...
- 与本项目参数映射：tau, SafeTorque / safety constraints
- 实现难度：medium
